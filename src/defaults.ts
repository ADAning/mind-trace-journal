...
function normalizePluginSettings(value) {
  ...
  return {
    ...
    weeklyReportMinimumDays: Math.min(7, Math.max(4, Math.round(Number(source.weeklyReportMinimumDays) || DEFAULT_SETTINGS.weeklyReportMinimumDays))),
    ...
    monthlyReportMinimumWeeks: Math.min(5, Math.max(1, Math.round(Number(source.monthlyReportMinimumWeeks) || DEFAULT_SETTINGS.monthlyReportMinimumWeeks))),
